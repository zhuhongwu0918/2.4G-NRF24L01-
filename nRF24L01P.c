...
//	retValue |= HAL_nRF24L01P_SetPRXWidth(nRF, nRF->PayloadWidth, nRF_DATA_PIPE_0);
	...
	retValue |= HAL_nRF24L01P_DynACK(nRF, nRF_DISABLE);//nRF_ENABLE，关闭自动应答
	retValue |= HAL_nRF24L01P_ACKPayload(nRF, nRF_ENABLE);
	retValue |= HAL_nRF24L01P_DynPayload(nRF, nRF_ENABLE);//打开动态长度;
	//2 //RW, Enables Dynamic payloard length；1 //RW, Enable Payload with ACK；0 //RW, Enables the W_TX_PAYLOAD_NOACK command

	retValue |= HAL_nRF24L01P_RXPipe(nRF, nRF_DATA_PIPE_0, nRF_ENABLE);
	retValue |= HAL_nRF24L01P_DPLPipe(nRF, nRF_DATA_PIPE_0, nRF_ENABLE);//使能管道0动态包长度
	...
	retValue |= HAL_nRF24L01P_TXRX(nRF, nRF_STATE_TX);//zhhw??nRF_STATE_RX
...
//	HAL_nRF24L01P_CE_High(nRF);
	...
	SEGGER_RTT_printf(0, "Registers have been setted!\r\n");
	return HAL_OK;
}
...
HAL_StatusTypeDef HAL_nRF24L01P_TransmitPacketACK(nRF24L01P *nRF, uint8_t *Data, nRF_DATA_PIPE Pipe)
{
	...
	while(nRF->Busy);	// TODO: Add *timeout* functionality等待中断产生
	SEGGER_RTT_printf(0, "jump out of Interrupted!\r\n");
	return HAL_OK;
}
...
HAL_StatusTypeDef HAL_nRF24L01P_SendCommand(nRF24L01P *nRF, uint8_t Command, uint8_t *TxBuffer, uint8_t *RxBuffer, uint8_t Size)
{
	...
	for(uint8_t i = 0; i < Size; i++)
	{
		...
		SEGGER_RTT_printf(0, "TxBuffer:%s\n",TxBuffer);
